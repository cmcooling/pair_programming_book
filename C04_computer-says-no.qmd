...
One disadvantage of Jupyter notebooks is that they are based on interactive programming, meaning that variables are kept into memory between runs of different code snippets --- even if the corresponding code is then deleted. This can lead to issues trying to debug an unusual error, where the problem code *has since been deleted*[^C04-ftn-str]. Other notebooks than Jupyter can solve this (by updating the entire notebook state on changes) such as [marimo](https://marimo.io/) for Python or [Pluto](https://plutojl.org/) for Julia, but Jupyter is by far the most readily available solution.
